    ...
    for it in xrange(num_iters):
      ...
      for p in model:
        ...
        if update == 'sgd':
          ...
          #####################################################################
          velocity = momentum * self.step_cache[p] - learning_rate * grads[p]
          dx = velocity
          self.step_cache[p]=velocity
          ...
          # Don't forget to also update the step_cache[p].                    #
          #####################################################################
          v_prev = self.step_cache[p] # back this up
          v = momentum * self.step_cache[p] - learning_rate * grads[p] # velocity update stays the same
          dx=v
          self.step_cache[p] = -momentum * v_prev + (1 + momentum) * v  # position update changes form
